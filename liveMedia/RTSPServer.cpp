  ...
  do {
    ...
    if (fIsMulticast) {
	  ...
	  snprintf((char*)ourClientConnection->fResponseBuffer, sizeof ourClientConnection->fResponseBuffer,
		   ...
		   "CSeq: %s\r\n"
		   "%s"
		   "Transport: MP2T/RTP/UDP;multicast;destination=%s;source=%s;port=%d-%d;ttl=%d\r\n"
		   "Session: %08X\r\n\r\n",
		   ourClientConnection->fCurrentCSeq,
		   dateHeader(),
		   destAddrStr.val(), sourceAddrStr.val(), ntohs(serverRTPPort.num()), ntohs(serverRTCPPort.num()), destinationTTL,
		   fOurSessionId);	// RTP/AVP => MP2T/RTP/UDP，电信对rtsp协议修改
	  break;
        case RTP_TCP:
	  ...
	  snprintf((char*)ourClientConnection->fResponseBuffer, sizeof ourClientConnection->fResponseBuffer,
		   ...
		   "CSeq: %s\r\n"
		   "%s"
		   "Transport: MP2T/RTP/UDP;unicast;destination=%s;source=%s;client_port=%d-%d;server_port=%d-%d\r\n"
		   "Session: %08X\r\n\r\n",
		   ourClientConnection->fCurrentCSeq,
		   dateHeader(),
	  ...
	  snprintf((char*)ourClientConnection->fResponseBuffer, sizeof ourClientConnection->fResponseBuffer,
		   ...
		   "CSeq: %s\r\n"
		   "%s"
		   "Transport: MP2T/RTP/TCP;unicast;destination=%s;source=%s;interleaved=%d-%d\r\n"
		   "Session: %08X\r\n\r\n",
		   ourClientConnection->fCurrentCSeq,
		   dateHeader(),
		   destAddrStr.val(), sourceAddrStr.val(), rtpChannelId, rtcpChannelId,
		   fOurSessionId);	// RTP/AVP/TCP => MP2T/RTP/TCP，电信对rtsp协议修改
	  break;
	}
        case RAW_UDP: {
	  snprintf((char*)ourClientConnection->fResponseBuffer, sizeof ourClientConnection->fResponseBuffer,
